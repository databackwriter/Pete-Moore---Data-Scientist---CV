---
title: "Curriculum Vitae - Pete Moore"
# author: "Pete Moore"
# date: "`r Sys.Date()`"
header-includes:
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \fancyhead[CO,CE]{Tel:07810698243 e:pete@lookatyourdata.ai}
...
for (row in 1:nrow(section.df)){
  ...
  if (!section.name %in% section.ignore){
    ...
    if(section.name %in% section.astable){
      ...
      o.df <- select(o.df, opportunityname, company, state, startpoint, endingpoint)
      o.df <- rename(o.df, Position=opportunityname, Institution=company, Place=state, From=startpoint, To=endingpoint)
      print(knitr::kable(o.df, caption = paste(section.name,' details')))#select(iris, "Sepal.Length", "Sepal.Width")
      cat("\n")
    }
  }
}
