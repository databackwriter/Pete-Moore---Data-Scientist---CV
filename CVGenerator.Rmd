...
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \fancyhead[CO,CE]{Tel:07810698243 mailto:pete@lookatyourdata.ai}
# - \fancyfoot[CO,CE]{Power BI Data Viz of this CV: http://bit.ly/PeteMooreDataViz}
# - \fancyfoot[LE,RO]{\thepage}
output: pdf_document
---

Power BI Data Viz of this CV: http://bit.ly/PeteMooreDataViz 
...
for (row in 1:nrow(section.df)){
  ...
  for (row in 1:nrow(f.df)){
    if (section.id != 1){
      section.astable <- c("Education", "Summer work")
...
```
<!-- ## Education and summer work -->
```{r, echo=FALSE, eval=TRUE}
# opportunity.sql <- "dbo.usp_PM_opportunity_Getopportunitys"
# opportunity.query <- dbSendQuery(con, opportunity.sql)
# opportunity.list <- dbFetch(opportunity.query)
# opportunity.df <- data.frame(opportunity.list)
